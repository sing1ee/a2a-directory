...
| [Emboss](https://getemboss.ai) | [Emboss](https://getemboss.ai) | Production A2A agent for PDF forms. Turns a flat PDF into a fillable form, finds a blank government form by name, fills it from data or documents or for every row of a spreadsheet, proposes and commits answers with a receipt, reads a filled form back, verifies it, faxes it, and composes, merges, splits and rotates PDFs. 23 skills over JSON-RPC with streaming at `/a2a`; bearer sk_ key, OAuth 2.1, or anonymous pay-per-job with x402 (USDC on Base). [Agent Card](https://api.getemboss.ai/.well-known/agent-card.json) \| [Docs](https://getemboss.ai/docs/a2a) | N/A |
